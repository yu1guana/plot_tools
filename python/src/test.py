...
def test_Plot_5():
    canvas= pt.Canvas(PlotRange=[[0,2*np.pi], [-1,1]])
    canvas.ListLinePlot(gl_2d_1_graph_datas)
    # canvas.Export(gl_graphs_directory+"test_5.eps")

def test_Plot_6():
    canvas= pt.Canvas(SerifFont=True)
    canvas.ListLinePlot(gl_2d_1_graphs_datas_list, PlotLegendNames=["sin($x$)","sin(x+$\pi/4$)"])

def test_Plot_7():
    # canvas= pt.Canvas(Ticks=True, LabelNames=["x", "y"], TitleName="Example", PlotRange=[0,2*np.pi])
    canvas= pt.Canvas(SerifFont=False, Ticks=True, LabelNames=[r'$x$', r"$y$"], TitleName="Example", PlotRange=[[0,2*np.pi], [0,1]], MajorGrid=True, MinorGrid=True)
